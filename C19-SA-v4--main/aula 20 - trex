...
function setup() {
 ...
 console.log(message)
  ...
  gameOver = createSprite(height/2,width/2 - 40);
  gameOver.addImage(gameOverImg);
  
  restart = createSprite(height/2,width/2);
...
function spawnObstacles(){
 if (frameCount % 60 === 0){
   var obstacle = createSprite(600,height - 60,10,width);
...
function spawnClouds() {
  ...
  if (frameCount % 60 === 0) {
    var cloud = createSprite(600,height - 100,40,width);
    cloud.y = Math.round(random(100,width - 300));
